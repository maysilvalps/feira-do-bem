...
// =====================================================
// TEXTO DIGITADO SOZINHO (TYPEWRITER) NO TOPO
// =====================================================
document.addEventListener("DOMContentLoaded", () => {
  iniciarEfeitoDigitador();
});

function iniciarEfeitoDigitador() {
  const dynamicSpan = document.querySelector(".chalk-underline");
  if (!dynamicSpan) return;

  // Frases ligeiramente mais curtas e diretas para encaixar perfeitamente na tela do celular
  const palavras = ["virar lixo.", "desperdício.", "virar fumaça.", "descarte."];
  let palavraIndex = 0;
  let caractereIndex = palavras[0].length;
  let apagando = false;
  ...
  function digitar() {
    ...
    if (!apagando) {
      dynamicSpan.textContent = palavraAtual.slice(0, caractereIndex++);
      if (caractereIndex > palavraAtual.length) {
        apagando = true;
        setTimeout(digitar, 2200); // Tempo da palavra na tela
        return;
      }
    } else {
      dynamicSpan.textContent = palavraAtual.slice(0, caractereIndex--);
      if (caractereIndex < 0) {
        ...
        caractereIndex = 0;
        setTimeout(digitar, 400); // Pausa antes da próxima palavra
        return;
      }
    }
